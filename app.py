...
class Store:
    ...
    def init(self):
        ...
        with self.conn() as c: c.executescript(SCHEMA); c.execute("UPDATE tasks SET state='queued', started_at=NULL WHERE state='running'"); self.prune_tasks(c)
...
def create_app(config=None):
    ...
    def inject(): return {'csrf': csrf(), 'clock': clock, 'mask': mask, 'localtime': lambda value: localtime(value, app.config['TZ']), 'timezone': app.config['TZ'], 'state_labels': {'queued': '排队中', 'running': '执行中', 'success': '成功', 'failed': '失败'}}
    @app.before_request
    def verify_csrf():
    ...
    def plan_detail(pid):
        ...
        body = '''<div class=card><h2>{{plan.name}}</h2><form method=post class=toolbar><input type=hidden name=csrf value='{{csrf}}'><input name=at type=time required><input name=low type=number min=1 placeholder='固定值或最小值' required><input name=high type=number min=1 placeholder='最大值（可选）'><button>添加时间点</button></form><p class=muted>同一时间点仅执行一次；随机区间在入队时抽取。</p><div class=table-wrap><table class=responsive><thead><tr><th>时间</th><th>目标</th><th></th></tr></thead><tbody>{% for p in points %}<tr><td data-label=时间>{{clock(p.at_minute)}}</td><td data-label=目标>{{p.low_steps}}{% if p.low_steps != p.high_steps %} – {{p.high_steps}}{% endif %}</td><td data-label=操作><form class=inline method=post action='{{url_for("delete_point",pid=pid,point_id=p.id)}}'><button class=danger>删除</button></form></td></tr>{% else %}<tr><td colspan=3 class=empty>还没有时间点。</td></tr>{% endfor %}</tbody></table></div><form method=post action='{{url_for("delete_plan",pid=pid)}}'><button class=danger onclick='return confirm("删除计划？已分配账户将变为未分配。")'>删除计划</button></form></div>'''

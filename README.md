...
每次手动或定时上传前，系统会查询 Zepp Life 设备列表。已有设备时使用列表中的第一个设备 ID；成功返回空列表时，系统会为该账户生成并保存一个虚拟设备 MAC 和设备 ID，再调用绑定接口创建“小米手环 5 NFC”记录。虚拟身份和令牌一起加密保存，后续执行、备份恢复后都会复用，不会每次生成新的设备。

“测试”只登录并查询设备，不会创建虚拟设备或上传步数；空列表会显示“当前账户尚无绑定设备”。设备查询或绑定接口失败时，本次上传会使用内置 fallback 设备 `DA932FFFFE8816E7`，并在任务记录中显示失败原因；绑定失败后下一次上传会使用同一虚拟身份再次尝试绑定。接口接受绑定请求不代表微信或支付宝已经确认同步。
...
数据库保存在 `/opt/steps/data/app.db`。日常灾难恢复请优先使用网站“设置 → 灾难恢复备份”：设置至少 12 字符的独立备份密码后下载 `.stepsbak` 文件。该文件已加密，包含账户凭据、计划、设置、执行记录和当天随机时间快照；请将文件和密码分开保管。

在新服务器完成部署并登录后，打开同一位置上传备份文件、填写备份密码并确认覆盖即可恢复。恢复不依赖旧服务器的 `APP_SECRET`；系统会用新服务器的密钥重新保护凭据。恢复完成后所有账户都会停用，未完成任务会标记为跳过，不会自动重放。请先检查时区、代理和计划，再在“账户”页批量启用需要继续运行的账户。备份不能恢复其创建之后才产生的数据。

仍可在维护前采用停服复制数据库的方式备份：
